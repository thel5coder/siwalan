...
class KepemilikanRepository implements IKepemilikanRepository
{
    ...
    public function paginationData(PaginationParam $param)
    {
        ...
        $result->setTotalCount(KepemilikanModel::count());

        //get data
        if ($param->getKeyword() == '') {

            if ($skipCount == 0) {
                $data = KepemilikanModel::take($param->getPageSize())
                    ->orderBy($sortBy, $sortOrder)
                    ->get();
            } else {
                $data = KepemilikanModel::skip($skipCount)->take($param->getPageSize())
                    ->orderBy($sortBy, $sortOrder)
                    ->get();
            }
        } else {
            if ($skipCount == 0) {
                $data = KepemilikanModel::where('nama_pemilik', 'like', '%' . $param->getKeyword() . '%')
