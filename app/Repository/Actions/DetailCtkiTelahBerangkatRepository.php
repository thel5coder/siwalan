...
use App\Models\DetailCtkiTelahBerangkatModel;
...
class DetailCtkiTelahBerangkatRepository implements IDetailCtkiTelahBerangkatRepository
{

    public function create($input)
    {
        $detail = new DetailCtkiTelahBerangkatModel();
    ...
    {
        $detail = DetailCtkiTelahBerangkatModel::find($input['id']);
        $detail->lapor_id = $input['laporId'];
        $detail->nama_jabatan = $input['namaJabatan'];
        $detail->kode_jabatan = $input['kodeJabatan'];
        $detail->jml_sd = $input['jmlSd'];
    ...
    {
        return DetailCtkiTelahBerangkatModel::find($id)->delete();
    }

    public function read($id)
    {
        return DetailCtkiTelahBerangkatModel::find($id);
    }
    ...
    {
        return DetailCtkiTelahBerangkatModel::where('lapor_id','=',$laporId)->get();
    }

    public function deleteByLapor($laporId)
    {
        return DetailCtkiTelahBerangkatModel::where('lapor_id','=',$laporId)->delete();
    }
    ...
    public function paginationByLapor(PaginationParam $param, $laporId)
    {
        ...
        $result->setTotalCount(DetailCtkiTelahBerangkatModel::where('lapor_id','=',$laporId)
            ->count());
        ...
        if ($param->getKeyword() == '') {
            ...
            if ($skipCount == 0) {
                $data = DetailCtkiTelahBerangkatModel::where('lapor_id','=',$laporId)
    ...
    {
        if($id ==''){
            $result = DetailCtkiTelahBerangkatModel::where('nama_jabatan','=',$jabatan)->count();
        }else{
            $result = DetailCtkiTelahBerangkatModel::where('id','<>',$id)->where('nama_jabatan','=',$jabatan)->count();
        }
    ...
    {
        if($id ==''){
            $result = DetailCtkiTelahBerangkatModel::where('kode_jabatan','=',$kodeJabatan)->count();
        }else{
            $result = DetailCtkiTelahBerangkatModel::where('id','<>',$id)->where('kode_jabatan','=',$kodeJabatan)->count();
        }
